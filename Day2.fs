...
module Solution2  =
    ...
    let parseInputString (inputString:string) =
        let getCharacterFromArray ((number:int), (password:string)) = password.[number - 1]
        inputString.Split([|' '; ':'; '-'|]) //{|1;3;a;;abcde|} 
        |> fun array -> 
                    {
            char1 = getCharacterFromArray (int array.[0], string array.[4] )
            char2 = getCharacterFromArray (int array.[1], string array.[4] )
            letter = char array.[2]
            password = string array.[4]
            }

    let checkNumber passwordPolicy =
        match passwordPolicy.letter with
        |letter when letter = passwordPolicy.char1 && letter <> passwordPolicy.char2 -> true
        |letter when letter = passwordPolicy.char2 && letter <> passwordPolicy.char1 -> true
        |_ -> false
